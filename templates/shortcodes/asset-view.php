		...
		<section class="almgr-asset-view__loan-history" aria-label="<?php esc_attr_e( 'Loan history', 'asset-lending-manager' ); ?>">
			<details class="almgr-collapsible almgr-collapsible--history">
				...
				<div class="almgr-collapsible__body">
					...
					$almgr_history_caption      = __( 'Recent loan history entries for this asset', 'asset-lending-manager' );
					$almgr_history_partial_path = trailingslashit( ALMGR_PLUGIN_DIR ) . 'templates/shortcodes/partials/asset-history-table.php';

					if ( $almgr_history_page_url ) {
						$almgr_history_full_url = add_query_arg(
							'almgr_asset_id',
							$almgr_asset_id,
							$almgr_history_page_url
						);
					}
					?>
					...
					<?php if ( ! empty( $almgr_history ) ) : ?>
						...
						<?php endif; ?>
						<?php if ( $almgr_history_total > 10 && ! empty( $almgr_history_full_url ) ) : ?>
							<p class="almgr-history-more">
								<a href="<?php echo esc_url( $almgr_history_full_url ); ?>" class="almgr-link">
									<?php
									printf(
										/* translators: %d: total number of history entries. */
										esc_html__( 'View full history (%d entries)', 'asset-lending-manager' ),
										(int) $almgr_history_total
									);
									?>
								</a>
							</p>

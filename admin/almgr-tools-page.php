...
<div class="wrap">
	...
	<h2 class="nav-tab-wrapper">
		<?php foreach ( $almgr_tabs as $almgr_tab_key => $almgr_tab_label ) : ?>
			<?php
			$almgr_tab_url = add_query_arg( 'tab', $almgr_tab_key, $almgr_tools_page_url );
		...
		<?php else : ?>
			...
			<?php if ( ! empty( $almgr_users_import_report ) ) : ?>
				...
				$almgr_report_errors = isset( $almgr_users_import_report['errors'] ) && is_array( $almgr_users_import_report['errors'] )
					...
					: array();
				?>
				<h3><?php esc_html_e( 'Last Import Report', 'asset-lending-manager' ); ?></h3>
				<p>
					<?php
					printf(
	...
	<?php elseif ( 'export' === $almgr_current_tab ) : ?>
		...
		<p><?php esc_html_e( 'CSV export tools will be available in this tab.', 'asset-lending-manager' ); ?></p>

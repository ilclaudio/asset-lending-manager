<?php
/**
 * Integration Tests Bootstrap.
 *
 * @package AssetLendingManager
 */

// Get the tests directory.
$almgr_tests_dir = getenv( 'WP_TESTS_DIR' );

if ( ! $almgr_tests_dir ) {
	$almgr_tests_dir = 'C:/WordpressDEV/wordpress-develop/tests/phpunit';
}

// Verify the path exists.
if ( ! file_exists( $almgr_tests_dir . '/includes/functions.php' ) ) {
	die( "WordPress test library not found at: {$almgr_tests_dir}\n" );
}

// Give access to tests_add_filter() function.
require_once $almgr_tests_dir . '/includes/functions.php';

/**
 * Manually load the plugin being tested.
 */
function almgr_manually_load_plugin() {
	require dirname( __DIR__, 1 ) . '/asset-lending-manager.php';
}

// Add filter to load plugin before WordPress loads.
tests_add_filter( 'muplugins_loaded', 'almgr_manually_load_plugin' );

// Start up the WP testing environment.
require $almgr_tests_dir . '/includes/bootstrap.php';
